...
def log_in(current_page):
    """
    :param pages.BasePage current_page:
    """
    # Start from Main page
    driver = current_page.get_driver()
    ...
    driver = current_page.get_driver()
    return pages.AccountPage(driver=driver, url=test_data.ACCOUNT_PAGE)
...
def check_product_in_cart(items, product_desc):
    """
    :param list items:
    :param str product_desc:
    """
    product_desc, _ = format_product_desc(product_desc)

    for item in items:
        if product_desc in item.text:
            break
    else:
        pytest.fail(f'Product {product_desc} not found in cart items')


def test_login(start_page):
    # Start from Main page and log in to reach Account page
    current_page = log_in(start_page)

    # Check current page after log in
    assert current_page.get_current_url() == test_data.ACCOUNT_PAGE
    ...
    assert user_name == test_data.USER_NAME


def test_add_product_to_cart(start_page):
    ...
    check_product_in_cart(items=items, product_desc=product_desc)


def test_add_products_to_cart(start_page):
    # Start from Main page and log in
    current_page = log_in(start_page)
    ...
    for i in range(random.randint(1, number_of_products)):
        ...
        desc, price = format_product_desc(product.text)
        count += 1
        exp_total += price

    # Check number of products and total price
    cart_total = main_page.get_cart_total()
    number, act_total = format_cart_total(cart_total)
    assert count == number
